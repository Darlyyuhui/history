...
<script type="text/javascript">
    ...
    function delParam(){
		$.getJSON("${root}/property/cabinfo/del_earchparams",
			function(data) {
				//alert(data.result);
			});
	}
    ...
    function exportXls(){
        var name = $("#s_name").val();
        var ip = $("#s_ip").val();
        window.location.href="${root}/property/cabinfo/exportXls/${menuid}/?search_name="+name+"&search_ip="+ip;
    }
...
<div class="conten_box" style="height:690px;overflow:hidden;margin-left:180px;">
    ...
    <div class="search-area" style="padding:6px 0 4px;">
      <form class="form-inline" action="${root}/property/cabinfo/list/${menuid}/?searchFlag=1" method="post" 
      style="margin:0; padding:0 6px;" target="myIframe">
        <table class="table-pad-td" width="100%">
          <tr>
            <td class="search_item_td">设备名称</td>
            <td><input title="支持 设备名称 模糊查询" id="s_name" style="width:90%;" name="search_name" value="${cab.name}" type="text" maxlength="20" placeholder="设备名称" /></td>
            <!-- 
            <td class="search_item_td">建设厂家</td>
            <td><select id="dev-select" name="search_companyId" style="width:96%;">
                <option value="">请选择</option>
                <c:forEach items="${companyInfoMap}" var="keyValue">
                  <option value="${keyValue.key}" ${keyValue.key==device.companyId?'selected':''}>${keyValue.value}</option>
                </c:forEach>
              </select></td>
               -->
             <td class="search_item_td">IP 地址</td>
            <td><input title="支持 IP地址 模糊查询" id="s_ip" style="width:90%;" name="search_ip" value="${cab.ip}" type="text" maxlength="15" placeholder="设备IP" /></td>
    ...
    <div class="mar_5">
      ...
      <div class="row-fluid">
      	<div class="pull-left ztree_box" style="margin:0;height:382px; width:180px; overflow:auto; position:fixed; left:0;margin-top: 225px;z-index: 10001;">
    ...
    <h4 id="modifyTitle" class="xtcs_h4" style="margin: 0;"></h4>
  	<div class="modal-body">
	    <div class="conten_box">
			<form id="modifyInputForm" class="form-inline" action="${root}/property/cabinfo/doModify" method="post" style="margin:0;">
...
<script type="text/javascript">
	...
	$("#showInMap").click(function(){
		...
		$.ajax({
			url: "${root}/property/cabinfo/jsonlist/${menuid}/",
			type: "POST",
			data: params,
	...
	$("#showInTable").click(function() {
		...
		$("#myIframe").attr("src","${root}/property/cabinfo/sublist/${menuid}/");
	});
...
<script type="text/javascript">
	...
	function showList(){
		window.location.href = "${root}/property/cabinfo/list/${menuid}/?${searchParams}";
    }
	...
	function treeOnlick(event, treeId, treeNode, clickFlag){
		...
		chooseId = treeNode.id;
       	myIframe.window.location.href="${root}/property/cabinfo/sublist/${menuid}/?search_roadId="+chooseId+"&searchFlag=2";
    }
