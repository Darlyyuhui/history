...
<div class="conten_box" style="margin-left: 180px;">
	<form id="inputForm" class="form-inline" action="${root}/property/serverinfo/doUpdate?assetid=${asset.id}&ismodify=<%=ismodify%>" method="post">
		<h4 class="xtcs_h4" style="margin:0;">服务器信息-修改
		</h4>
				...
				<div class="tab-pane mar_5 active" id="tab1" style="min-height:500px;">
					<table class="table tingche-table table-border-rl table-border-bot">
						<tr>
							...
							<td>
								<input style="width:160px;" type="text" id="name" name="name" maxlength="30" value="${server.name}" class="required">
								<font color="red">*</font>
							</td> 
							<td class="device_td_bg3">服务器IP：</td>
							<td>
								<input style="width:160px;" type="text" id="serverip" name="serverip" value="${server.serverip}" ip="true" maxlength="15" class="required">
			                    <font color="red">*</font>
							</td>	
						</tr>
						<tr>
							...
							<td>
								<input style="width:160px;" type="text" id="model" name="model" value="${server.model}" class="required" maxlength="20">
								<font color="red">*</font>
							</td>		
							...
							<td>
								<select style="width:37%;min-width: 170px;" id="type" name="type" placeholder="服务器类别" class="required">
								    ...
								    <option value="机架式" ${'机架式'==server.type?'selected':''}>机架式</option>
								    <option value="塔式" ${'塔式'==server.type?'selected':''}>塔式</option>
						        </select>
								<font color="red">*</font>
							</td> 
						</tr>
						<tr>
					  	    ...
					  	    <td>
					  	      <input type="text" id="orgNames" name="orgNames" value="${server.orgNames}" readonly onclick="showMenu('orgNames');" 
								  placeholder="所属部门" class="required" style="width:160px;"/><strong style="color:red">&nbsp;*</strong>
						      <input type="hidden" id="orgId" name="orgId" value="${server.orgId}"/> 
					  	    </td>
					          ...
					          <select style="width:170px;padding:4px;overflow:hidden;" id="factoryId" name="factoryId" class="required">
					            ...
					            <c:forEach items="${factoryList}" var="factory">
					              <option value="${factory.id}" ${factory.id==server.factoryId?'selected':''}>${factory.name}</option>
					            </c:forEach>
						      </select>
						      <font color="red">*</font>
					        </td>
				...
				<div class="tab-pane mar_5" id="tab2" style="min-height:500px;">
					<table class="table tingche-table table-border-rl table-border-bot" style="border:1px solid #ccc;">
						<tr>
							...
							<td>
								<input style="width:160px;" type="text" id="cpuModel" name="cpuModel" value="${server.cpuModel}" class="required" maxlength="20">
								<font color="red">*</font>
							</td> 
							<td class="device_td_bg3">CPU类型：</td>
							<td>
								<input style="width:160px;" type="text" id="cpuType" name="cpuType" value="${server.cpuType}" maxlength="20">
							</td> 
						</tr>
						<tr>
							<td class="device_td_bg3">CPU数量：</td>
							<td>
								<input style="width:160px;" type="text" id="cpuNum" name="cpuNum" value="${server.cpuNum}" digits="true" maxlength="2" class="required">
			                    <font color="red">*</font>（颗）
							</td>
							<td class="device_td_bg3">CPU核心数：</td>
							<td>
								<input style="width:160px;" type="text" id="cpuCoreNum" name="cpuCoreNum" value="${server.cpuCoreNum}" digits="true" maxlength="2" class="required">
						 ...
						 <tr>
							...
							<td>
								<input style="width:160px;" type="text" name="threadNum" value="${server.threadNum}" digits="true" maxlength="3">
							</td>
							<td class="device_td_bg3">三级缓存：</td>
							<td>
								<input style="width:160px;" type="text" name="threeCache" value="${server.threeCache}" digits="true" maxlength="3">（MB）
							</td>			
						</tr>
					</table>
				</div>
				...
				<div class="tab-pane mar_5" style="min-height:500px;" id="tab3" >
					<table class="table tingche-table table-border-rl table-border-bot" id="TbData">
						<tr>
							...
							<td>
								<input style="width:160px;" type="text" name="ramType" value="${server.ramType}" class="required" maxlength="10">
								<font color="red">*</font>
							</td> 
							<td class="device_td_bg3">内存容量：</td>
							<td>
								<input style="width:160px;" type="text" id="ramVolume" name="ramVolume" value="${server.ramVolume}" digits="true" class="required" maxlength="5">
								<select style="width:5%;min-width: 60px;" name="ramUnit" placeholder="内存单位">
									<option value="GB" ${'GB'==server.ramUnit?'selected':''}>GB</option>
								    <option value="TB" ${'TB'==server.ramUnit?'selected':''}>TB</option>
						        </select>
								<font color="red">*</font>
							</td> 
						</tr>
						<tr>
							...
							<td>
								<input style="width:160px;" type="text" name="diskPortType" value="${server.diskPortType}" maxlength="10">
							</td> 
							<td class="device_td_bg3">硬盘容量：</td>
							<td>
								<input style="width:160px;" type="text" id="diskVolume" name="diskVolume" value="${server.diskVolume}" digits="true" class="required" maxlength="5">
								<select style="width:5%;min-width: 60px;" name="diskUnit" placeholder="内存单位">
									<option value="GB" ${'GB'==server.diskUnit?'selected':''}>GB</option>
								    <option value="TB" ${'TB'==server.diskUnit?'selected':''}>TB</option>
						        </select>
								<font color="red">*</font>
							</td> 
						</tr>
						<tr>
							...
							<td colspan="3">
								<textarea rows="3" style="width:80%;" name="note" maxlength="100" >${server.note}</textarea>
							</td>	
						</tr>
					</table>
				</div>
					...
					<div class="pull-left" style="width:76%;">
				  		<table class="table table-border-rl table-border-bot tingche-table">
				  		  ...
				  		  <tr>
				  		    ...
				  		    <td>
				  		    	<input style="width:200px;" id="guaranteetimestr" name="guaranteetimestr" value="<fmt:formatDate value="<%=date %>" pattern="yyyy-MM-dd" />" class="required" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true,maxDate:'new Date()'})" type="text" placeholder="保修时间" />
					          	<font color="red">*</font>
					        </td>
				  		  </tr>
				  		  <tr>
...
<script type="text/javascript">
	...
	function showList(){
		window.location.href = "${root}/property/serverinfo/list/${menuid}/?isgetsession=1&page=${page}";
	}
