...
<script type="text/javascript">
    ...
    function exportXls(){
        ...
        window.location.href="${root}/property/ftpinfo/exportXls/${menuid}/?time="+new Date().getTime()+"&search_name="+name+"&search_ftptype="+ftptype+"&search_ip="+ip;       
    }
...
<div class="conten_box" style="height:690px;overflow:hidden;margin-left:180px;">
	<div class="search-area" style="padding:6px 0 4px;">
		<form class="form-inline" action="${root}/property/ftpinfo/list/${menuid}/" method="post" style="margin:0;padding:0 6px;">
			<table width="100%">
			  <tr>
			    <td class="td40">FTP名称</td>
			    <td><input title="支持 FTP名称 模糊查询" style="width:90%;max-width:144px;" id="name" name="search_name" value="${ftp.name}" maxlength="30" type="text" placeholder="FTP名称" /></td>
			    <td class="td40">FTP类型</td>
			    <td><input title="支持 FTP类型 模糊查询" style="width:90%;max-width:144px;" id="ftptype" name="search_ftptype" value="${ftp.ftptype}" maxlength="15" type="text" placeholder="FTP类型" /></td>
			    <td class="td40">IP地址</td>
			    <td><input title="支持 IP地址 模糊查询" style="width:90%;max-width:144px;" id="ip" name="search_ip" value="${ftp.ip}" maxlength="15" type="text" placeholder="IP地址" /></td>
	...
	<div class="mar_5">
	  ...
	  <div class="row-fluid" style="overflow: auto;">
		  	<table class="table table-striped table-bordered table-condensed table-style" id="table">
				...
				<tbody id="tbody">
					<c:forEach items="${pageList.result}" var="ftp">
						<tr onclick="rowOnclick(this)" data="${ftp.id}">
							<td style="text-align:center;"><input type="checkbox" value="${ftp.id}" name="select-chk" /></td>
							...
							<td class="center">
								<btn:authorBtn menuid="${menuid}" text="查看">
									<a href="javascript:viewById('${ftp.id}')"> <i class="icon-th-list"></i>查看</a>
								</btn:authorBtn>
								<btn:authorBtn menuid="${menuid}" text="修改">
									<a href="javascript:updateById('${ftp.id}')"> <i class="icon-edit"></i>修改</a>
								</btn:authorBtn> 
								<btn:authorBtn menuid="${menuid}" text="删除">
									<a href="javascript:delById('${ftp.id}')"> <i class="icon-remove"></i>删除</a>
								</btn:authorBtn>
							</td>
						</tr>
					</c:forEach>
...
<script type="text/javascript">
	...
	function showList(){
		window.location.href = "${root}/property/ftpinfo/list/${menuid}/?page=${current}&isgetsession=1";
	}

	function add() {
		var page='${current}';
		window.location.href = "${root}/forward/property/ftpinfo/add/?menuid=${menuid}&page=" + page;
	}

	function updateById(id,ismodify) {
		var page = '${current}';
		window.location.href = "${root}/property/ftpinfo/update/" + id
				+ "/${menuid}/?page=" + page;
	}
	
	function viewById(id){
		var page='${current}';
		window.location.href="${root}/property/ftpinfo/view/"+id+"/${menuid}/?page="+page;
	}
	...
	function delById(ids) {
		if(confirm("删除后无法恢复，并同步删除资产信息、服务商分配信息及工单信息等，确定删除吗？")){
			var url = "${root}/property/ftpinfo/delete/" + ids + "/";
			$.ajax({
				type : 'delete',
				url : url,
				dataType : "json",
