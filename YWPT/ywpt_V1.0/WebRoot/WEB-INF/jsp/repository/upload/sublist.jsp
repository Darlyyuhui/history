...
<!-- 重置查询输入框结束 -->
<div class="conten_box" style="height:650px;overflow:hidden;margin-left:180px;">
	<div class="search-area" style="padding: 6px 0 4px;">
		<form class="form-horizontal" id="intelForm" name="intelForm"
			action="${root}/repository/upload/sublist/${menuid}/" method="post" 
					...
					<c:forEach items="${pageList.result}" var="knowledge">
						<tr data="${knowledge.id}">
							...
							<td>
								...
								<btn:authorBtn menuid="${menuid}" text="下载">
									<a href="javascript:downloadById('${knowledge.id}')"><i class="icon-download"></i>下载 </a>
								</btn:authorBtn>
								<c:if test="${knowledge.applyflag==0||knowledge.verifyresult==2}">
									<btn:authorBtn menuid="${menuid}" text="删除">
...
<script type="text/javascript">
	...
	function viewById(id){
	    var page = '${current}';
		var url = "${root}/repository/upload/view/"+id+"/${menuid}/?page=" + page;
	...
	function updateById(id){
		window.location.href = "${root}/repository/upload/update/" + id + "/${menuid}/";
	}
	
	//显示列表信息
	function showList(){
		window.location.href = "${root}/repository/upload/sublist/${menuid}/";
	}
	
	//删除数据
	function delById(ids){
		if(confirm("删除后数据将无法恢复，确定要删除吗？")){
   			var url="${root}/repository/upload/delete/"+ids+"/";
			$.ajax( {
				type : 'delete',
				url : url,
				dataType : "json",
	...
	function downloadById(id){
		var url="${root}/repository/upload/isFileExist/" + id + "/";
		$.ajax({
			type : 'GET',
			url : url,
			dataType : "text",
	...
	function add(){
		...
		var chooseId = parent.add();
   		...
   		window.location.href = "${root}/forward/repository/upload/add/?menuid=${menuid}&page="+page+"&pid="+chooseId;
	}
	...
	function applyById(id){
		var url = "${root}/repository/verify/doApply/"+id+"/upload/?menuid=${menuid}";
		window.location.href = url;
	}
</script>
