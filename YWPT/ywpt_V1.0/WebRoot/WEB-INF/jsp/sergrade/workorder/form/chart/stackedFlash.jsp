...
<html>
	...
	<script LANGUAGE="JavaScript">
			...
			function generateXML(){			
				...
				strXML = (document.getElementById("PA").checked==true)?(strXML + getProductXML('stackedXml')):(strXML);
				strXML = (document.getElementById("PB").checked==true)?(strXML + getProductXML('solveStackedXml')):(strXML);
				strXML = (document.getElementById("PC").checked==true)?(strXML + getProductXML('nosolveStackedXml')):(strXML);
				strXML = strXML + "</chart>";
				return strXML;
			}
			function getProductXML(productIndex){		
				var productXML;
				if(productIndex=="stackedXml"){
				    productXML = "${stackedXml}";
				}
				if(productIndex=="solveStackedXml"){
				    productXML = "${solveStackedXml}";
				}
				if(productIndex=="nosolveStackedXml"){
				    productXML = "${nosolveStackedXml}";
				}
				return productXML;			
			}
			 	    			...
			 	    			<td style="border:1px solid #CCCCCC;" id="2d_id">
				 	    			<div id="2dcheckbox" style="float:left;">
										<div style="display:none">
											<B>请选择:</B>
											<INPUT id = "PA"  TYPE='Checkbox' name='PA' onClick="JavaScript:updateChart('chart1Id');" checked />&nbsp;总工单数&nbsp;&nbsp;
											<INPUT id = "PB"  TYPE='Checkbox' name='PB' onClick="JavaScript:updateChart('chart1Id');" checked />&nbsp;已解决&nbsp;&nbsp;
											<INPUT id = "PC"  TYPE='Checkbox' name='PC' onClick="JavaScript:updateChart('chart1Id');" checked />&nbsp;未解决&nbsp;&nbsp;
