	...
	<div class="table_box">
    	<div class="btn-group-wrap mar_b5">
		  <div class="btn-group pull-right">
		    ...
		    <btn:authorBtn menuid="${menuid}" text="工单遗留">
			<button class="btn btn-small" onclick="leave();">
				<i class="icon-ban-circle"></i>工单遗留
			</button>
		    </btn:authorBtn>
		  </div>
		  <div class="clear"></div>
				...
				<tbody id="tbody">
					<c:forEach items="${pageList.result}" var="workorder">
						<tr onclick="rowOnclick(this)" data="${workorder.id}">
							...
							<c:if test="${workorder.offaccount==null && workorder.isleave == 0}">
								<btn:authorBtn menuid="${menuid}" text="工单关闭">
							...
							<c:if test="${workorder.offaccount==null && workorder.isleave == 0}">
								<btn:authorBtn menuid="${menuid}" text="工单遗留">
									<a href="javascript:showReasonDialog('${workorder.id}','leave')"> <i class="icon-ban-circle"></i>工单遗留</a>
								</btn:authorBtn>
							</c:if>
							</td>
						</tr>
...
jQuery.support.cors = true;
	...
	function showReasonDialog(id, type) {
		...
		if(type == 'close')
		{
			...
			$("#title").html("工单关闭说明窗口");
			$("#input_form").attr("action", "${root}/alarm/workclose/off");
		}else{
			$("#title").html("工单遗留说明窗口");
			$("#input_form").attr("action", "${root}/alarm/workclose/leave");
		}
	}
	function showList(){
		window.location.href = "${root}/alarm/workclose/list/${menuid}/?page=${current}&isgetsession=1";
	}
