...
<div class="conten_box">
	<div class="search-area" style="padding:6px 0 4px;">
		<form class="form-inline" action="${root}/alarm/workreassign/list/${menuid}/" method="post" style="margin:0;padding:0 6px;">
			<table width="100%">
			  <tr>
	...
	<div class="table_box">
    	<div class="btn-group-wrap mar_b5">
		  <div class="btn-group pull-right">
		    <btn:authorBtn menuid="${menuid}" text="工单转派">
			<button class="btn btn-small" onclick="assign();">
				<i class="icon-repeat"></i>工单转派
			</button>
		    </btn:authorBtn>
		  </div>
		  <div class="clear"></div>
					...
					<c:forEach items="${pageList.result}" var="workorder">
						<tr onclick="rowOnclick(this)" data="${workorder.id}">
							...
							<td class="center">
								...
								<btn:authorBtn menuid="${menuid}" text="工单转派">
									<a href="javascript:assignById('${workorder.id}')"> <i class="icon-repeat"></i>工单转派</a>
								</btn:authorBtn> 
							</td>
						</tr>
					</c:forEach>
...
<script type="text/javascript">
	...
	function assign() {
		...
		if (ids.length == 0) {
			showMessage("请选择要转派的工单记录。");
		} else if (ids.length > 1) {
			showMessage("请选择单条记录进行转派。");
		} else {
			assignById(ids);
		}
	}
	
	function assignById(id) {
		var page='${current}';
		window.location.href="${root}/alarm/workreassign/showassign/"+id+"/${menuid}/?page="+page;
	}
