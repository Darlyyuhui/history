...
<div class="conten_box" style="height:675px;overflow:hidden;">
	<div class="search-area" style="padding:6px 0 4px;">
		<form class="form-inline" action="${root}/alarm/workappraise/list/${menuid}/" method="post" style="margin:0;padding:0 6px;">
			<table width="100%">
			  <tr>
	...
	<div class="table_box">
    	<div class="btn-group-wrap mar_b5">
		  <div class="btn-group pull-right">
		  	<btn:authorBtn menuid="${menuid}" text="工单评估">
			<button class="btn btn-small" onclick="appraise();">
				<i class="icon-star"></i>工单评估
			</button>
		    </btn:authorBtn>
		  </div>
		  <div class="clear"></div>
					...
					<c:forEach items="${pageList.result}" var="workorder">
						<tr onclick="rowOnclick(this)" data="${workorder.id}">
							...
							<td class="center">
								...
								<c:if test="${workorder.note==null}">
									<btn:authorBtn menuid="${menuid}" text="工单评估">
										<a href="javascript:appraiseById('${workorder.id}')"> <i class="icon-star"></i>工单评估</a>
									</btn:authorBtn>
								</c:if>
							</td>
						</tr>
...
<!--
	function appraise() {
		...
		if (ids.length == 0) {
			showMessage("请选择要评估的工单记录。");
		}else if(ids.length > 1){
		   showMessage("请选择单条记录操作");
		}else {
			appraiseById(ids);
		}
	}
	
	function appraiseById(id){
		var page='${current}';
		window.location.href="${root}/alarm/workappraise/appraise/"+id+"/${menuid}/?page="+page;
	}
