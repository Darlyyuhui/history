...
$(function(){
	ioscurrentpage = parent.getIosCurrentPage();
	app_id = parent.getApp_id_i();
	document.getElementById("app_name").focus();
	
	$.ajax({
		type : "POST",
		url: "../../../../iosversion/getIosVersionByID",
		data: {
			"app_id": app_id
		},
			...
			$.ajax({
				...
				success:function(returnedData){
					...
					if(jo0.check =="true"){
						...
						parent.iosflush(ioscurrentpage);
						parent.$("#ios_check").prop("checked",false);
...
function checkVersionName(){
	...
	if(""==ver_name.replace(/\s/g,"")){
		...
		if(ver_name.length<=10&&ver_name.length>=1){
			$.ajax({
				type: "POST",
				url: "../../../../iosversion/checkVersionName",
				data:{
					"param":ver_name
				},
				success:function(returnedData){
