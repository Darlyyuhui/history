...
var longitude_message;
var latitude_message;
var unit_pic="";
var comp_key;
...
$(function(){
	...
	$.ajax({
		...
		success: function(returnedData){
			...
			cunit_name = jo0.unit_name;
			comp_key = jo0.comp_key;
			var ctype_key = jo0.type_key;
			var type_key = jo0.type_key;
			$("#unit_name").val(jo0.unit_name);
			$("#unit_location").val(jo0.unit_location);
			$("#unit_longitude").val(jo0.unit_longitude);
			$("#unit_latitude").val(jo0.unit_latitude);
			$("#unit_remark").val(jo0.unit_remark);
			textarealength("#unit_remark",100);
			unit_pic = jo0.unit_pic;
		 	var help = unit_pic.split(";");
		 	$("#img_1").attr('src',"../../../../"+help[0]);
		 	if(""!=jo0.unit_longitude&&""!=jo0.unit_latitude){
		 		map.centerAndZoom(new BMap.Point(jo0.unit_longitude,jo0.unit_latitude), 13);
			 	var marker = new BMap.Marker(new BMap.Point(jo0.unit_longitude, jo0.unit_latitude));
			...
			$.ajax({
				...
				success:function(returnedData){
					...
					for(var m=0;m<jar0.length;m++){
						if(jar0[m].type_key==ctype_key){
							str0 +="<div class='model-select-text' id='type_key_select' data-value='"+jar0[m].type_key+"'>"+jar0[m].type_name+"</div>"+ 
							"<ul class='model-select-option'>";
							break;
						}
					}
					for(var j=0;j<jar0.length;j++){
						if(jar0[j].type_key==ctype_key){
