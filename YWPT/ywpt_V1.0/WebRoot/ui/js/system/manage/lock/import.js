	...
	if(filename.length>0){
		...
		if("xls" == filetype ){
			...
			$.ajaxFileUpload({
	            ...
	            data: {
	            	"exceltype": "1",//exceltype为1时表示导入锁具信息
	            	"comp_key": comp_key
	            },
	            success : function(data, status) //服务器成功响应处理函数
	            {
	            	...
	            	if(jo0[0].message=='y'){
	            		...
	            		for(var j=0;j<jo0[0].list.length;j++){
							...
							"<td title='"+jo0[0].list[j].lock_id+"'>"+jo0[0].list[j].lock_id+"</td>" +
							"<td title='"+jo0[0].list[j].lock_name+"'>"+jo0[0].list[j].lock_name+"</td>" +
							"<td title='"+jo0[0].list[j].unit_name+"'>"+jo0[0].list[j].unit_name+"</td>" +
							"<td title='"+jo0[0].list[j].lock_model+"'>"+jo0[0].list[j].lock_model+"</td>"+
							"<td title='"+jo0[0].list[j].lock_remark+"'>"+jo0[0].list[j].lock_remark+"</td>";
						}
	            		...
	            		if(""!=jo0[0].repeat_message){
	            			count++;
	            			str1 += "<thead>" +
	        				"<tr class='text-l'><th>"+message_value[count]+"锁编号重复</th>" +
	        				"</tr></thead><tbody><tr class='text-l'><td>"+jo0[0].repeat_message+"</td><tr></tbody>";
	            		}
	            		if(""!=jo0[0].rows_message){
	            			count++;
	            			str1 += "<thead>" +
	        				"<tr class='text-l'><th>"+message_value[count]+"锁编号不符合要求</th>" +
	        				"</tr></thead><tbody><tr class='text-l'><td>"+jo0[0].rows_message+"</td><tr></tbody>";
	            		}
	            		if(""!=jo0[0].id_message){
	            			count++;
	            			str1 += "<thead>" +
	        				"<tr class='text-l'><th>"+message_value[count]+"锁编号已存在</th>" +
	        				"</tr></thead><tbody><tr class='text-l'><td>"+jo0[0].id_message+"</td><tr></tbody>";
	            		}
	            		if(""!=jo0[0].unit_message){
	            			count++;
	            			str1 += "<thead>" +
	        				"<tr class='text-l'><th>"+message_value[count]+"所属单元不存在</th>" +
	        				"</tr></thead><tbody><tr class='text-l'><td>"+jo0[0].unit_message+"</td><tr></tbody>";
	            		}
	            		if(""!=jo0[0].name_message){
	            			count++;
	            			str1 += "<thead>" +
	        				"<tr class='text-l'><th>"+message_value[count]+"锁名称不符合要求</th>" +
	        				"</tr></thead><tbody><tr class='text-l'><td>"+jo0[0].name_message+"</td><tr></tbody>";
	            		}
	            		if(""!=jo0[0].model_message){
	            			count++;
	            			str1 += "<thead>" +
	        				"<tr class='text-l'><th>"+message_value[count]+"锁型号不符合要求</th>" +
	        				"</tr></thead><tbody><tr class='text-l'><td>"+jo0[0].model_message+"</td><tr></tbody>";
	            		}
	            		...
	            		layer.closeAll('loading');
	            		layer.alert('请选择锁具表格模板,录入数据后再导入!',{
		...
		parent.layer.msg('请选择文件!',{icon:2,shade:0.3,time:1000});
	}
}
function sure(){
