...
$(function(){
	...
	$("#authorize_name").val(i18Frame.getauthapply()+now_date);
	$("#logmin").val(now_date);
	$("#logmax").val(now_date);
...
function Authorize(){
	...
	key_value = keyIframe.window.Key_key();
	...
	if(""==start_date){
		...
		layer.closeAll('loading');
		layer.tips(i18Frame.getinput_apply_name(),'#authorize_name',{
			tips:3,time:2000
		});
	...
	}else if(0==key_value.length){
		layer.closeAll('loading');
		keyIframe.layer.tips(i18Frame.getchoose_auth_key(),$("#keyIframe").contents().find("#key_id"),{
			tips:3,time:2000
		});
		...
		$.ajax({
			...
			success: function(returnedData1){
				...
				if(jo1.check =="true"){
					$.ajax({
						type: "POST",
						url: "../../../authorize/insertAuthorizeApply",
						data: {
						...
						success: function(returnedData){
							...
							layer.closeAll('loading');
							if(jo0.check =="true"){
								layer.confirm(i18Frame.getapply_success()+','+i18Frame.getkeep_templet()+'?',{
									btn: [i18Frame.getok(),i18Frame.getcancel()],icon:7,title:i18Frame.getinfo(),closeBtn:0
								},function(index){
									$.ajax({
										type: "POST",
										url: "../../../templet/insertTemplet",
										data: {
											"authorize_name":authorize_name,
											"start_date":start_date,
											"end_date":end_date,
										...
										success: function(returnedData){
											var open_click = window.parent.$("#myapply");
								...
								layer.alert(i18Frame.getapply_failed()+'!',{icon:2,shade:0.3,title:i18Frame.getinfo()});
								return;
							}
						}
					});
