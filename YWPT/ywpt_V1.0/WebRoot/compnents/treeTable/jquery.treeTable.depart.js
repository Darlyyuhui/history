...
//        }
        ...
        function initRelation($trs, hideLevel) {
            ...
            $trs.each(function (i) {
                ...
                var depth =  $tr.attr('level');//getDepth(this.id);
                $tr.attr('depth',depth);
                //格式化节点
				formatNode(this);
        ...
        function formatNode(tr) {
            var $cur = $(tr);
            var id = tr.id;
            //console.log("cmap:"+cMap[id].length+"-----"+cMap[id]);
            //console.log("pmap:"+pMap[cMap[id]]);
            //-------------下面一大段都是获取$preSpan---------
            if (cMap[id] =='0'||cMap[id].length==1) {
                //如果是顶级节点，则没有prev_sp
            	//console.log("is top: true");
                var $preSpan = $('<span class="prev_sp"></span>');
            } else {
            	//console.log("is top: false");
                //先判断是否有上一个兄弟节点
