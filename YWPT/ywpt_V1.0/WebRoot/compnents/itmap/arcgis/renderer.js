...
itmap.arcgis.renderer = function(){
	return function() {
		var api = {
			...
			if(util.checkParam(graphicLayer)) {
				...
				for(var k in result) {
					...
					var symbol = new esri.symbol.PictureMarkerSymbol(
							"images/map/arrow/"+k.toLowerCase()+".png",16,16);
					for(var m in result[k]) {
						var p = result[k][m];
						mp.addPoint({"x": p[0], "y": p[1]});
					}
					var g = new esri.Graphic(mp, symbol);
					graphicLayer.add(g);
					gs.push(g);
				}
		...
		function addRouteResult(array, graphicLayer) {
			...
			if(util.checkParam(graphicLayer)) {
				...
				for(var i=0; i<result.length; i++) {
					...
					line.addPath(result[i][0]);
					var g = new esri.Graphic(line);
					var symbol;
					var linecolor;
					var linewidth;
					if(result[i][1] <= maxNum/3) {
						...
						// 判断此级别的子级别
						linecolor = new dojo.Color([142,142,234]);
						...
						linecolor = new dojo.Color([102,102,170]);
						...
						// 第三级别383860--56,56,96
						linecolor = new dojo.Color([56,56,96]);
