	...
	return function(){
		...
		function setEditMode(mode){
			...
			if(mode.match(_editVertices)){
				_mode |= SuperMap.Control.ModifyFeature.RESHAPE;
			}
			if(mode.match(_rotateReg)){
				_mode |= SuperMap.Control.ModifyFeature.ROTATE;
			}
			if(mode.match(_moveReg)){
				_mode |= SuperMap.Control.ModifyFeature.DRAG;
			}
			if(mode.match(_scale)){
				_mode |= SuperMap.Control.ModifyFeature.RESIZE;
			}
		}
		...
		function activate(){
			...
			_edit = new SuperMap.Control.ModifyFeature(_layer,_options);
