MapFactory.Define("MapFactory/Draw",[
	...
	return function(){
		...
		function activate(){
			...
			if(!_tempDrawLayer){
				_tempDrawLayer = new OpenLayers.Layer.Vector();
		...
		function _drawPoint(){
			return function(pixel){
		        ...
		        this.point = new OpenLayers.Feature.Vector(geometry,{},_markerSym);
		...
		function _snappingInit(){
			...
			_snapResourceLayer = new OpenLayers.Layer.Vector(_snapResourceLayerid);
			if(_ids){
		...
		function _snappingHandler(){
			...
			_snappingManager = new OpenLayers.Control.Snapping({
		...
		function _iteratorLayers(index){
			if(index < _urls.length){
				...
				for(var i=0,len=_resGraphicList.length;i<len;i++){
					_resGraphicList[i] = new OpenLayers.Feature.Vector(_geoUtil.convertFromMapFactory(_resGraphicList[i].geo));
				}
