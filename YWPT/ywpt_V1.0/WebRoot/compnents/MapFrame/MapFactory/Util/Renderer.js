MapFactory.Define("MapFactory/Util/Renderer*",[
	...
	return function(){
		...
		// 标注箭头
		function addRouteResultArrow(array, layerId, callback) {
			/**
			...
			for(var k in result) {
				...
				for(var m in result[k]) {
					// 这里应该添加点对象到多点对象
					var p = result[k][m];
					var point = new geoFun({type: geoType.POINT, points: p[0]+","+p[1]});
					var gm = graphicFun({geo:point, symbol:{url:"images/map/arrow/"+k.toLowerCase()+".png", size:16}});
					if(isAdd)gm.addToLayer(layerId);// 添加到layerId图层
					gs.push(gm.getGraphic());
				}
			}
		...
		// list为分析结果线的点的集合[[x1,y1],[x2,y2]]    layerId添加到地图上的graphiclayer
		function addRouteResult(array, layerId) {
				...
				for(var j=0; j<result.length; j++) {
					...
					if((first[0] == temp1[0] && first[1] == temp1[1] && 
						...
						if(util.isBlongLine(result[j][0], first)) {
							...
							if(util.isBlongLine([first,second], result[j][0][0]) && util.isBlongLine([first,second], result[j][0][1])) {
								flag = false;
								result[j][1] += 1;// 更新重复数量
								maxNum = Math.max(result[j][1], maxNum);
								// 添加截断的两条线到list并跳出
			...
			if(util.checkParam(layerId)) {
				...
				for(var i=0; i<result.length; i++) {
					...
					var linePoints = "";
					for(var mm=0,mml=result[i][0].length; mm<mml; mm++) {
						linePoints += result[i][0][mm][0]+","+result[i][0][mm][1];
						if((mm+1) < mml)linePoints += ",";
					}
					var polyline = new geoFun({type: geoType.POLYLINE, points: linePoints});
					
					var linecolor;
					var linewidth;
					if(result[i][1] <= maxNum/3) {
						// 第一级别8e8eea--142,142,234
						// 判断此级别的子级别
						linecolor = "#8e8eea";
					...
					var gm = graphicFun({geo:polyline, symbol:{outLineWidth: linewidth, outLineColor: linecolor, outLineStyle: "Solid"}});
					gm.addToLayer(layerId);// 添加到layerId图层
				}
			}
			return result;
		}
