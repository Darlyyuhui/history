...
MapFactory.OverLayerMap=new HashMap();
...
(function() {
    ...
    (function() {
         ...
         * 参数的意思：beforeBegin&lt;span&gt;afterBegin 取值为beforeBegin,afterBegin,beforeEnd,afterEnd  this is span! beforeEnd&lt;/span&gt; afterEnd <br />
    ...
    DrawingManager.prototype._bindPolylineOrPolygon = function() {
        ...
        var startAction = function (e) {   	
            ...
            if (points.length == 1) {
                ...
                var overlayid=MapFactory.GenerateID();             
                MapFactory.OverLayerMap.put(overlayid,overlay);
        ...
        if (this._enableCalculate && BMapLib.GeoUtils) {
            ...
            switch (type) {
                case "[object Polyline]":
				     unit="km"
                    result.data = BMapLib.GeoUtils.getPolylineDistance(overlay)/1000;
                    break;
                case "[object Polygon]":
                	unit="km"+"<sup>2</" + "sup>";
                    result.data = BMapLib.GeoUtils.getPolygonArea(overlay)/1000000;
                    break;
                case "[object Circle]":
				     unit="km"+"<sup>2</" + "sup>";
                    var radius = overlay.getRadius();
                    result.data = Math.PI * radius * radius/1000000;
                    break;
            }
            //一场情况处理
            ...
            result.label = this._addLabel(point, result.data+unit);
        }
        return result;
    }
    ...
    DrawingManager.prototype._addLabel = function (point, content) {
        ...
        var overlayid=MapFactory.GenerateID();
        MapFactory.OverLayerMap.put(overlayid,label);
        return label;
    }
