MapFactory.Define("MapFactory/MapManager",[
	...
	return function(conf){
		...
		var _mapHandler = null,
			...
			_initExtent = _mapConfig.initExtent,
		...
		if(!_mapHandler){
			...
			_baseLayer = new SuperMap.Layer.TiledDynamicRESTLayer(_mapConfig.layers.baseMap.id,_mapConfig.layers.baseMap.url,{},{});
		...
		function _addLayer(){
			...
			_baseLayerLabels = new SuperMap.Layer.TiledDynamicRESTLayer(_mapConfig.layers.baseMapLabels.id,_mapConfig.layers.baseMapLabels.url,{
				transparent: true, cacheEnabled: true
			},{});
			_baseLayerLabels.id = _mapConfig.layers.baseMapLabels.id;
			//_mapHandler.addLayer(_baseLayerLabels);
			_baseLayerLabels.events.on({"layerInitialized": _addLabelsLayer});
		...
		function _dbClickEvt(){
			...
			var clickControl = new SuperMap.Control.Click();
