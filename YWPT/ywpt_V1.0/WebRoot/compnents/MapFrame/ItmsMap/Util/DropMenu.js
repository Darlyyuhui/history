 ...
 */
MapFactory.Define("ItmsMap/Util/DropMenu*",function(){
	return function(){
		...
		function _initMenu(menu){
			for(var i = 0,len = menu.length;i<len;i++){
				...
				if(menu[i].style){ // 如果存在指定的风格，则添加指定风格
					_div.addClass(menu[i].style);
				}else{
					_div.addClass("mapMenuList");
				}
				if(menu[i].childNode && menu[i].childNode.length){
		...
		function _initBind(){
			...
			_span.bind("mouseenter",function(){
				...
				if(_thisUL.children().length){
					_thisUL.css("marginLeft",$(this).width() - _thisUL.width() + 8);
