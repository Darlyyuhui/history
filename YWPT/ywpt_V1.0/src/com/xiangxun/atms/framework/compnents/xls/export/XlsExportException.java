package com.xiangxun.atms.framework.compnents.xls.export;
...
/**
 * <p>Title: XlsExportException</p>
 * <p>Description: Excel文件导出的异常类定义
 * <br>
 * <br>主要提供的构造函数接口如下：
 * <br><br>
 * 1、以资源配置文件中定义的配置项作为参数，构造异常类：
 * <pre>
 *     public XlsExportException(String errorCode)
 * </pre>
 * 2、以资源配置文件中定义的配置项和一个字符串格式化参数作为参数，构造异常类：
 * <pre>
 *     public XlsExportException(String errorCode, String errorArg)
 * </pre>
 * 3、以资源配置文件中定义的配置项和格式化参数数组作为参数，构造异常类：
 * <pre>
 *     public XlsExportException(String errorCode, Object[] errorArgs)
 * </pre>
 * 4、以附加异常（一般是当时捕获的异常对象）作为参数，构造异常类：
 * <pre>
 *     public XlsExportException(Throwable root)
 * </pre>
 * 5、以资源配置文件中定义的配置项和附加异常（一般是当时捕获的异常对象）作为参数，构造异常类：
 * <pre>
 *     public XlsExportException(String errorCode, Throwable root)
 * </pre>
 * 6、以资源配置文件中定义的配置项、一个字符串格式化参数和附加异常（一般是当时捕获的异常对象）作为参数，构造异常类：
 * <pre>
 *     public XlsExportException(String errorCode, String errorArg, Throwable root)
 * </pre>
 * 7、以资源配置文件中定义的配置项、格式化参数数组和附加异常（一般是当时捕获的异常对象）作为参数，构造异常类：
 * <pre>
 *     public XlsExportException(String errorCode, Object[] errorArgs, Throwable root)
 * </pre>
...
public class XlsExportException extends RuntimeException {
	//序列化标识
	...
	public XlsExportException(String errorCode, String errorArg) {
		super();
		this.errorCode = errorCode;
		if (!StringUtils.isNull(errorArg)) {
	...
	public String getMessage() {
		...
		try {
			...
			errorMsg = "Invoke XlsExportException.getMessage() except: "+ ex.toString();
		}
	...
	public String getMessage(Locale locale) {
		...
		try {
			...
			errorMsg = "Invoke XlsExportException.getMessage("
