...
import com.xiangxun.atms.module.gis.util.GeoUtil;
...
public class MapQueryCtl {	
	 ...
	 * 
	 * */
	@RequestMapping("getRoadByNames")
	@ResponseBody
	@LogAspect(desc="查询道路")
	public List<LayerBean> getRoadByNames(HttpServletRequest req) {
		String[] values = req.getParameterValues("roadName");
		//List<String> names = Arrays.asList(values);
		String where = "";
		
		if (values.length > 0) {
			where = "name like '%" + values[0] + "%'";
		}
		
		for (int i = 1; i < values.length; i++) {
			where = where + "or name like '%" + values[i] + "%'";
		}

		List<LayerBean> results = geoLayerOperation.getByWhere(LayerEnum.ROAD_LINE, where);
	 ...
	 * 根据几何范围查询道路
	 */
	@RequestMapping("getRoadByGeometry")
	@ResponseBody
	@LogAspect(desc="查询道路")
	public List<LayerBean> getRoadByGeometry(String geometryText) {
		List<LayerEnum> layers = new ArrayList<LayerEnum>();
		layers.add(LayerEnum.ROAD_LINE);
		List<LayerBean> results = spatialOperationService.identify(geometryText, layers);
	...
	@ResponseBody
	@LogAspect(desc="查询所有设备")
	public List<LayerBean> getAllDevices() {
		List<LayerBean> list = geoLayerOperation.getAll(LayerEnum.CROSS);		
		return list;
	}
	 ...
	 * 根据几何范围查询设备
	 * */
	@RequestMapping("getDeviceByGeometry")
	@ResponseBody
	@LogAspect(desc="以几何范围查询卡口")
	public List<LayerBean> getDeviceByGeometry(String geometryText) {
		List<LayerEnum> layers = new ArrayList<LayerEnum>();
		layers.add(LayerEnum.CROSS);
		List<LayerBean> list = spatialOperationService.identify(geometryText, layers);
	...
	public int addTmpGeometry(String type, String tmps) {
		...
		for(int i=0,len=jsonArr.size();i<len;i++){
			...
			if(attributes.containsKey("TYPE")){
				layerBean.setRoadId(attributes.getString("TYPE"));
			}
