...
MapFactory.Define("MapFactory/Util/Dialog*",function(){
	return function(conf) {
		 ...
		 */
		var _conf = {
            mapDiv : "",//mapdiv的名称，如果有则添加到该div下面，如果没有则添加到body下
		...
		function _initOuterDialog() {
			if(DialogBox == null || typeof DialogBox == "undefined"){
                ...
                if(_conf.mapDiv)document.getElementById(_conf.mapDiv).appendChild(DialogOuterBox);
                else document.body.appendChild(DialogOuterBox);
			}
		}
		...
		function _initMask(){
			...
			if(mask == null || typeof mask == "undefined"){
                ...
                document.getElementsByClassName("map-tools")[0].appendChild(mask);
			}
		}
		...
		function _initDialogFooter(){
			...
			if(DialogBoxConfirmButton == null || typeof DialogBoxConfirmButton == "undefined"){
				DialogBoxConfirmButton = document.createElement('button');
				DialogBoxConfirmButton.id = DialogBoxConfirmButtonID;
				DialogBoxConfirmButton.className = 'btn btn-info';
				DialogBoxConfirmButton.style.margin="0px 10px 0px 0px" ;
				$(DialogBoxConfirmButton).css({borderRadius:'3.5px'});
				DialogBoxConfirmButton.style.border="0px";
				DialogBoxFooter.appendChild(DialogBoxConfirmButton);
			}

			// 初始化对话框按钮二
			if(DialogBoxCancelButton == null || typeof DialogBoxCancelButton == "undefined"){
				DialogBoxCancelButton = document.createElement('button');
				DialogBoxCancelButton.id = DialogBoxCancelButtonID;
				DialogBoxCancelButton.className = 'btn btn-warning';
			    $(DialogBoxCancelButton).css({borderRadius:'3.5px'});
				DialogBoxCancelButton.style.border="0px";
		 ...
		 */
		function _getLeft() {
            if(_conf.mapDiv)return document.getElementById(_conf.mapDiv).clientWidth / 2 - DialogOuterBox.clientWidth / 2;
		 ...
		 */
		function _getTop() {
            if(_conf.mapDiv)return document.getElementById(_conf.mapDiv).clientHeight / 2 - DialogOuterBox.clientHeight / 2;
		...
		function _dragEnable(_event){
            ...
            if(_conf.mapDiv)scrollLeft = document.getElementById(_conf.mapDiv).scrollLeft;
			var scrollTop = Math.max(document.body.scrollTop,document.documentElement.scrollTop);
            if(_conf.mapDiv)scrollTop = document.getElementById(_conf.mapDiv).scrollTop;
			...
			if(DialogOuterBox.style.right != ''){
                ...
                if(_conf.mapDiv)p.left = document.getElementById(_conf.mapDiv).clientWidth - DialogOuterBox.clientWidth - p.right;
			...
			if(DialogOuterBox.style.bottom != ''){
                ...
                if(_conf.mapDiv)p.top = document.getElementById(_conf.mapDiv).clientHeight - DialogOuterBox.clientHeight - p.bottom;
				DialogOuterBox.style.top = p.top + 'px';
			}
		...
		function _move(_event){
			...
			var scrollTop = Math.max(document.body.scrollTop,document.documentElement.scrollTop);
            if(_conf.mapDiv) {
                scrollLeft = document.getElementById(_conf.mapDiv).scrollLeft;
                scrollTop = document.getElementById(_conf.mapDiv).scrollTop;
            }
		...
		function _dragEnd(){
			...
			if(DialogBoxTitle.children[0]){
				DialogBoxTitle.children[0].style.cursor = "pointer";
			}
			_dragEable = false;
		}
		...
		function setContent(content){
			...
			if(DialogBoxContent.clientHeight > _conf.maxHeight){
				...
				DialogBoxContent.style.overflow = "hidden";
			}
