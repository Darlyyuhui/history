	...
	return function(){
		...
		function _convertPointStrToArray(points){
			...
			for(var i=0;i<arrLen;i+=2){
				tmpArr.push(new OpenLayers.Geometry.Point(parseFloat(pointArr[i]),parseFloat(pointArr[i+1])));
			}
			return tmpArr;
		}
		...
		function _convertToPoint(points){
			var pointArr = points.split(",");
			return new OpenLayers.Geometry.Point(parseFloat(pointArr[0]),parseFloat(pointArr[1]));
		}
