	...
	return function(conf){
		...
		if(!_mapHandler){
			...
			var mapOptions = { 
					...
					maxExtent: new OpenLayers.Bounds(maxExtent.minX,maxExtent.minY,maxExtent.maxX,maxExtent.maxY),
					units: "unknown",
					controls: []
					};
			_mapHandler = new OpenLayers.Map(_conf.src, mapOptions );
			MapFactory._MapManagerResource[MapFactory.Engine] = _mapHandler;
			 _mapHandler.addControl(_controls["Navigation"]);
			 /*
			 _mapHandler.addControl(new OpenLayers.Control.PanZoomBar({
			...
			var baseLayer = new OpenLayers.Layer.WMS(
                ...
                {    
					tileOrigin: new OpenLayers.LonLat(-399.9999999999999, 399.9999999999999),
        ...
        function setBaseMapLayer(layer) {
            var _baseLayer = _mapHandler.getLayer(layer.id);
            if(_baseLayer) {
                _mapHandler.setBaseLayer(_baseLayer);
            }
            else {
                var _layer = new OpenLayers.Layer.WMS(
                    layer.id, _mapConfig.serviceUrl,
                    {
                        LAYERS: layer.url,
                        tiled:true,
                        format:"image/png8",
                ...
                _layer.id = layer.id;
                _mapHandler.addLayer(_layer);
                _mapHandler.setBaseLayer(_layer);
            }
        }
		...
		function centerAt(x,y,callback){
            // 获取当前地图中心，如果当前地图中心不变则放大一级别
            var _p = _mapHandler.getCenter();
            if(_p.lon == x && _p.lat == y) {
                zoomIn();
            }
            else {
