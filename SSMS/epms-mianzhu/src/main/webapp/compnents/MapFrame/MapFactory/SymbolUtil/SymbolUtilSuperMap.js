MapFactory.Define("MapFactory/SymbolUtil",[
	...
	return function(){
		...
		function _convertPointSymbol(symbolObj){
			...
			// pic marker
			if(symbolObj.url){
				pointSym = new SuperMap.Symbolizer.Point();
			...
			if(symbolObj.text){
				...
				pointSym = new SuperMap.Symbolizer.Text();
				pointSym.label = symbolObj.text;
				pointSym.fontFamily = fontfam;
				pointSym.fontSize = 12;
		...
		function _convertPolylineSymbol(symbolObj){
			var lineSym = new SuperMap.Symbolizer.Line();
		...
		function _convertPolygonSymbol(symbolObj){
			var polygonSym = new SuperMap.Symbolizer.Polygon();
		...
		function _convertPointToMapFactory(symbolObject){
			var _newSymbol = {};
			if(symbolObject instanceof SuperMap.Symbolizer.Point){
				...
				return _newSymbol;
			}else if(symbolObject instanceof SuperMap.Symbolizer.Text){
