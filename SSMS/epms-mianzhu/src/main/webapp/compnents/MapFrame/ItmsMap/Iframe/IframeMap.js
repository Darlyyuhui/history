MapFactory.Define("ItmsMap/Iframe/IframeMap*",[
	...
	"MapFactory/InfoWindowManager"
			...
			geoUtilFun, util, df, Tip, Parallel, MeasureMent, SymbolConfig, infoWindow){
	var gid = 1;
	return function(){
		var route = routeFun();
		var tip = Tip();
		...
		//缓存父页面的文档对象
		// var parentDoc = parent.document;弃用iframe，修改为当前文档对象
		var parentDoc = document;
		
		var vectorLayer;//不清除的layer，需要清除时需要指定具体的feature
		var drawLayer, roadLayer;
		...
		var api = {
            ...
            getSymbolConfig: function(){return SymbolConfig},//获取配置文件的样式
            getClass: function(classUrl, callback){
                if(!classUrl || !callback)return;
                MapFactory.Require([classUrl],function(clazz){
                    callback(clazz);
                });
            },// 根据class路径获取class
			...
			centerAt: centerAtByPointObj,// 定位地图中心
			...
			drawPoint: drawPoint,// 绘制点到地图上，添加到vectorLayer图层上，每次自动清空
            ...
            queryNature: queryNature,// 自然保护区信息查询
            queryWater: queryWater,// 水资源保护区信息查询
            queryNd: queryNd,// 矿区区信息查询
			...
			draw: draw,// 绘制
            drawPointForRoadInfo: drawPointForRoadInfo,// 开启绘制点，使用默认样式，并且每次绘制完成后清除图层
            addGeometry: addGeometry// 添加geometry到地图上，使用默认样式，并定位到地图中心
		};
		
		var pointStyle = {url: path+"/images/map/position.png", width: 34, height:40, xOffset: 8, yOffset: 20};
		...
		function centerAtByPointObj(x, y,level) {
			var l,lat,lon;
			if(typeof x === "string") {
				  lon = parseFloat(x);
		...
		function setPointImg(imgUrl, imgWidth, imgHeight, xoffset, yoffset) {
			if(!imgUrl) {
				...
				pointStyle = {url: path+"/images/map/position.png", width: 34, height:40, xOffset: 8, yOffset: 20};
			}
			else {
		...
		// 绘制多点，并添加到地图上
		function drawPoint(callback, style) {
	    	...
	    	drawControl.setDrawEndEvent(function(geo) {
	    		...
	    		if(isSnap && isRoadSnap) {
	    			getClosedPoint(geo, function(newGeo){
	    				if(newGeo) {
	    					_drawPointEnd(newGeo, callback, style);
	    				}
	    				else {
	    			...
	    			_drawPointEnd(geo, callback, style);
	    		}
	    	});
	       	drawControl.activate();
	    }
	    // drawPoint的辅助方法
	    function _drawPointEnd(geo, callback, style) {
            layerFun("defaultLayer").clear();
			...
			if(parentDoc.getElementById(mapx))parentDoc.getElementById(mapx).value = ps[0].substr(0, 8);
			if(parentDoc.getElementById(mapy))parentDoc.getElementById(mapy).value = ps[1].substr(0, 7);
		...
		// 绘制点，并添加到地图上
		function draw(type, callback, isAutoClear, layerId, style) {
            if(!type)return;
			if(!layerId)layerId = "defaultLayer";
			drawControl.deactivate();
			drawControl = drawFun();
		...
		// 绘制面，并添加到地图上
		function drawPolygon(callback, isAutoClear, layerId, style) {
			draw(geoType.POLYGON, callback, isAutoClear, layerId, style);
	    }
		
		// 绘制点，并添加到地图上
		function drawPointForRoadInfo(callback, layerId) {
			if(!layerId)layerId = "defaultLayer";
			drawControl.deactivate();
			drawControl = drawFun();
			if(isSnap && !isRoadSnap)drawControl.openSnapping(snapConfig);
	    	...
	    	drawControl.setDrawEndEvent(function(geo) {
	    		...
	    		if(isSnap && isRoadSnap) {
	    			getClosedPoint(geo, function(newGeo){
    					_drawPointForRoadEnd(newGeo, callback, layerId);
	    			});
	    		}
	    		else {
                    _drawPointForRoadEnd(geo, callback, layerId);
	    		}
	    	});
	       	drawControl.activate();
	    }
	    // drawPoint的辅助方法
	    function _drawPointForRoadEnd(geo, callback, layerId) {
        ...
        function queryNature(code, callback) {
            if(!callback)return;
            var _query = queryFun(), _natureurl=mapConfig.layers.naturePolygon.url;
            if(!_natureurl) {
                callback("");
                return;
            }
            _query.setUrl(_natureurl);
            if(code)_query.setCondition({CODE: code});
            _query.execute(function(result) {
                if(result && result.length) {
                    callback(result);
                ...
                if (typeof console != 'undefined' && console.log) console.log("查询自然保护区数据的时候，出错了！"+error);
                callback("");
            });
        }

        function queryWater(code, callback) {
            if(!callback)return;
            var _query = queryFun(), _waterurl=mapConfig.layers.waterPolygon.url;
            if(!_waterurl) {
                callback("");
                return;
            }
            _query.setUrl(_waterurl);
            if(code)_query.setCondition({CODE: code});
            _query.execute(function(result) {
                if(result && result.length) {
                    callback(result);
                ...
                if (typeof console != 'undefined' && console.log) console.log("查询水资源保护区数据的时候，出错了！"+error);
                callback("");
            });
        }

        function queryNd(code, callback) {
            if(!callback)return;
            var _query = queryFun(), _dnurl=mapConfig.layers.ndPolygon.url;
            if(!_dnurl) {
                callback("");
                return;
            }
            _query.setUrl(_dnurl);
            if(code)_query.setCondition({CODE: code});
            _query.execute(function(result) {
                if(result && result.length) {
                    callback(result);
                ...
                if (typeof console != 'undefined' && console.log) console.log("查询矿区数据的时候，出错了！"+error);
                callback("");
            });
        }
		...
		function popUpInfowindow(geo, title, content, width, height, callback) {
			var info = infoWindow({callback: callback});
			info.setWidth(width);
			info.setHeight(height);
            if(geo.type != "point")geo = graphicFun({geo:geo}).getCenter();
		...
		function addGeometry(geometry, layer, isCenter) {
			var graphic = graphicFun({geo:geometry, symbol:_getDefaultSymbol(geometry)});
			if(layer)graphic.addToLayer(layer);
			else graphic.addToLayer("vectorLayer");
			if(isCenter)centerAtByGraphicObject(graphic);
		}
