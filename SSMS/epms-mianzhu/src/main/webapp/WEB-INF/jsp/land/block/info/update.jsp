		...
		<table class="width-100">
			<tr>
				<td>
					...
					</div> <!--        地图显示区域 --> <!-- 地图统一样式引入 --> <tags:mapheader
						rotateImage="true"></tags:mapheader>
					...
					<div style="border: none; position: absolute; top: 0">
						<div class="profile-user-info profile-user-info-striped width-100" >
							...
							<div class="profile-info-row">
								...
								<div class="profile-info-value">
									<input type="text" id="regionName" readonly="readonly"
										value='<tags:xiangxuncache keyName="TREGION_NAME" id="${info.regionId }"/>'
										class="input-large required left-map-input-width"
										onclick="showRegion('regionId','regionName')" /> <span
										style="color: red">*</span>
								</div>
							</div>
							...
							<div class="profile-info-row">
								...
								<div class="profile-info-value">
									<input type="text" id="longitude" name="longitude"
										maxlength="12" value="${info.longitude }"
							...
							<div class="profile-info-row">
								...
								<div class="profile-info-value">
									<input type="text" id="soilTypeName" readonly="readonly"
										value='<tags:xiangxuncache keyName="LANDTYPE_NAME" id="${info.soilType }"/>'
							...
							<div class="profile-info-row">
								...
								<div class="profile-info-value">
									<input type="hidden" id="geoId" name="geoJson" /> <input
										type="text" id="area" name="area" maxlength="20"
										value="${info.area }" class="input-large left-map-input-width" />
								</div>
							</div>
							<div class="profile-info-row">
									...
									<select id="ownerId" name="ownerId"
										...
										<c:forEach items="${owners }" var="item">
											<option value="${item.id }"
												${info.ownerId eq item.id ? 'selected' : '' }>${item.name }</option>
										</c:forEach>
									</select>
								</div>
							</div>
						...
						<div class="clearfix form-actions width-100 pull-right"
							style="margin-top: 0; background: none; padding: 0; padding-left: 12px; bottom: 0">
...
<script>
	...
	function regionTreeCB() {
		$.get("${root}/bs/region/getLocation/" + $("#regionId").val() + "/",
				function(data) {
	...
	function _handleMeasurements(event) {
		...
		graphic = _Graphic({
			geo : geometry,
			symbol : _SymbolConfig.defaultPolygon,
			attributes : {}
		});
	...
	function clearDraw() {
		...
		MapFactory.Require([ "MapFactory/LayerManager" ],
