...
<div style="margin:10px 0">
    <form class="form-search" action="${root}/pollute/controll/list/${menuid}/" method="post">
        <div class="row">
            <div class="col-xs-12">
        ...
        <table id="table" class="table table-striped table-bordered table-hover table-style" style="text-align: center">
            ...
            <c:forEach items="${pageList.result}" var="item">
                <tr>
                    ...
                    <td>
                    	<div style="width:190px;height:22px;margin-top:2px;white-space:nowrap;overflow:hidden;-o-text-overflow:ellipsis;text-overflow:ellipsis;"
                    ...
                    <td>
                    	<div style="width:180px;height:22px;margin-top:2px;white-space:nowrap;overflow:hidden;-o-text-overflow:ellipsis;text-overflow:ellipsis;"
                          title="${item.dept }">${item.dept }</div>
                    </td>
                    <td><fmt:formatDate value="${item.executeTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                    <td>${item.isExecute eq '1' ? '是' : '否' }</td>
                    <td>
...
<div id="modal-table" class="modal fade" tabindex="-1" style="top:80px;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="advanForm" role="form" class="form-search" action="${root}/pollute/controll/list/${menuid}/" method="post">
            <div class="modal-body no-padding">
                    <div class="form-group" style="margin-top:10px;">
                        ...
                        <div class="col-sm-9">
                        	<input type="text" class="input-large gj-query-input" id="adv_name" name="search_name" value="${name }" placeholder="计划名称" />
                        </div>
                    </div>
                    <div class="form-group" style="margin-top:10px;">
                        <label class="col-sm-3 control-label no-padding-right"
                        ...
                        <div class="col-sm-9">
                        	<input type="text" class="input-large gj-query-input" id="adv_code" name="search_code" value="${code }" placeholder="计划编号" />
                        </div>
                    </div>
            </div>
...
<script type="text/javascript">
    function goList() {
        window.location.href = "${root}/pollute/controll/list/${menuid}/?isgetsession=1";
    }
    ...
    function add() {
        window.location.href = "${root}/pollute/controll/add/${menuid}/?page=${page}";
    }
    function updateById(id) {
        window.location.href = "${root}/pollute/controll/update/${menuid}/"+id+"/?page=${page}";
    }
    function showViewById(id) {
        window.location.href = "${root}/pollute/controll/showView/${menuid}/"+id+"/?page=${page}";
    }
    ...
    function deleteById(ids) {
        bootbox.confirm("删除后数据将无法恢复，确定要删除吗？",function(a) {
            if (!a) return;
            var url = "${root}/pollute/controll/doDelete/";
            $.ajax({
                type: 'post',
                url: url,
                data : "ids="+ids,
