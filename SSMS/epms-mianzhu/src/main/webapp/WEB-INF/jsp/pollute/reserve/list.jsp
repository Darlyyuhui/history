...
<div style="margin:10px 0">
    <form class="form-search" action="${root}/pollute/reserve/list/${menuid}/" method="post">
        <div class="row">
            <div class="col-xs-12">
                <div class="input-group">
                   	<input type="text" class="form-control search-query" id="name" name="search_name" value="${name }" placeholder="预案名称" />
        ...
        <table id="table" class="table table-striped table-bordered table-hover table-style" style="text-align: center">
            ...
            <c:forEach items="${pageList.result}" var="item">
                <tr>
                    ...
                    <td>${item.code }</td>
                    <td>${item.name }</td>
                    <td><tags:xiangxuncache keyName="Dic" typeCode="POLLUTE_RESERVE_TYPE" id="${item.accidentType }"/></td>
                    <td>${item.dept }</td>
                    <td>
...
<div id="modal-table" class="modal fade" tabindex="-1" style="top:80px;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="advanForm" role="form" class="form-search" action="${root}/pollute/reserve/list/${menuid}/" method="post">
            <div class="modal-body no-padding">
                    <div class="form-group" style="margin-top:10px;">
                        ...
                        <div class="col-sm-9">
                        	<input type="text" class="input-large gj-query-input" id="adv_name" name="search_name" value="${name }" placeholder="预案名称" />
                        </div>
                    </div>
                    <div class="form-group" style="margin-top:10px;">
                        <label class="col-sm-3 control-label no-padding-right"
                        ...
                        <div class="col-sm-9">
                        	<input type="text" class="input-large gj-query-input" id="adv_code" name="search_code" value="${code }" placeholder="预案编号" />
                        </div>
                    </div>
            </div>
...
<script type="text/javascript">
    function goList() {
        window.location.href = "${root}/pollute/reserve/list/${menuid}/?isgetsession=1";
    }
    ...
    function add() {
        window.location.href = "${root}/pollute/reserve/add/${menuid}/?page=${page}";
    }
    function updateById(id) {
        window.location.href = "${root}/pollute/reserve/update/${menuid}/"+id+"/?page=${page}";
    }
    function showViewById(id) {
        window.location.href = "${root}/pollute/reserve/showView/${menuid}/"+id+"/?page=${page}";
    }
    ...
    function deleteById(ids) {
        bootbox.confirm("删除后数据将无法恢复，确定要删除吗？",function(a) {
            if (!a) return;
            var url = "${root}/pollute/reserve/doDelete/";
            $.ajax({
                type: 'post',
                url: url,
                data : "ids="+ids,
