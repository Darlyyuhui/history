...
<div style="margin:10px 0">
    <form class="form-search" action="${root}/reg/back/list/${menuid}/" method="post">
        <div class="row">
            <div class="col-xs-12">
        ...
        <table id="table" class="table table-striped table-bordered table-hover table-style" style="text-align: center">
            ...
            <c:forEach items="${pageList.result}" var="item">
                <tr>
                    ...
                    <td>${item.code }</td>
                    <td><tags:xiangxuncache keyName="Dic" typeCode="SAMPLING_BACK_TYPE" id="${item.typeCode }"/></td>
                    <td><tags:xiangxuncache keyName="TREGION_FULL_NAME" id="${item.regionId }"/></td>
                    <td>${item.ambient }</td>
                    <td>${item.years }</td>
                    <td><tags:xiangxuncache keyName="Dic" typeCode="SAMPLING_BACK_SOURCE" id="${item.wallSource }"/></td>
...
<div id="modal-table" class="modal fade" tabindex="-1" style="top:80px;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="advanForm" role="form" class="form-search" action="${root}/reg/back/list/${menuid}/" method="post">
...
<script type="text/javascript">
    function goList() {
        window.location.href = "${root}/reg/back/list/${menuid}/?isgetsession=1";
    }
    ...
    function add() {
        window.location.href = "${root}/reg/back/add/${menuid}/?page=${page}";
    }
    function updateById(id) {
        window.location.href = "${root}/reg/back/update/${menuid}/"+id+"/?page=${page}";
    }
    function showViewById(id) {
        window.location.href = "${root}/reg/back/showView/${menuid}/"+id+"/?page=${page}";
    }
    ...
    function deleteById(ids) {
        bootbox.confirm("删除后数据将无法恢复，确定要删除吗？",function(a) {
            if (!a) return;
            var url = "${root}/reg/back/doDelete/";
            $.ajax({
                type: 'post',
                url: url,
                data : "ids="+ids,
    ...
    function checkByIds() {
        ...
        if (ids.length == 0) {
        	...
        	bootbox.confirm("确定要核查选中的数据吗？",function(a) {
                if (!a) return;
                var url = "${root}/reg/back/checkRegs/";
                $.ajax({
                    type: 'post',
                    url: url,
