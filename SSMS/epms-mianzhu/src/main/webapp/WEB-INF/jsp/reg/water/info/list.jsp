...
<div style="margin:10px 0">
    <form class="form-search" action="${root}/reg/water/list/${menuid}/" method="post">
        <div class="row">
            <div class="col-xs-12">
...
<div class="row">
    <div class="col-xs-12">
        ...
        <btn:authorBtn menuid="${menuid}" text="污染源">
            <button class="btn btn-sm btn-purple pull-left" onclick="goPollutes();"><i
                    class="icon-remove"></i>污染源
            </button>
        </btn:authorBtn>
    </div>
</div>
        ...
        <table id="table" class="table table-striped table-bordered table-hover table-style" style="text-align: center">
            ...
            <c:forEach items="${pageList.result}" var="item">
                <tr>
                    ...
                    <td>${item.code }</td>
                    <td><tags:xiangxuncache keyName="Dic" typeCode="SAMPLING_WATER_TYPE1" id="${item.typeCode }"/></td>
                    <td><tags:xiangxuncache keyName="Dic" typeCode="SAMPLING_WATER_TYPE2" id="${item.samplingType }"/></td>
                    <td><tags:xiangxuncache keyName="TREGION_FULL_NAME" id="${item.regionId }"/></td>
                    <td>${item.riversName }</td>
                    <td><tags:xiangxuncache keyName="Dic" typeCode="SAMPLING_WATER_POLLUTE1" id="${item.polluteSs }"/></td>
...
<div id="modal-table" class="modal fade" tabindex="-1" style="top:80px;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="advanForm" role="form" class="form-search" action="${root}/reg/water/list/${menuid}/" method="post">
            <div class="modal-body no-padding">
                    ...
                    <div class="form-group" style="margin-top:10px;">
                        ...
                        <div class="col-sm-9">
                        	<input type="text" class="input-large gj-query-input" id="adv_riversName" name="search_riversName" value="${riversName }" placeholder="河流名称" />
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function add() {
        window.location.href = "${root}/reg/water/add/${menuid}/?page=${page}";
    }
    function updateById(id) {
        window.location.href = "${root}/reg/water/update/${menuid}/"+id+"/?page=${page}";
    }
    function showViewById(id) {
        window.location.href = "${root}/reg/water/showView/${menuid}/"+id+"/?page=${page}";
    }
    ...
    function deleteById(ids) {
        bootbox.confirm("删除后数据将无法恢复，确定要删除吗？",function(a) {
            if (!a) return;
            var url = "${root}/reg/water/doDelete/";
            $.ajax({
                type: 'post',
                url: url,
                data : "ids="+ids,
    ...
    function checkByIds() {
        ...
        if (ids.length == 0) {
        	...
        	bootbox.confirm("确定要核查选中的数据吗？",function(a) {
                if (!a) return;
                var url = "${root}/reg/water/checkRegs/";
                $.ajax({
                    type: 'post',
                    url: url,
    ...
    function goPollutes() {
    	var ids = getSelectedValue();
        if (ids.length == 0) {
            showMessage("请选择要管理污染源的记录。");
        } else if (ids.length > 1) {
        	showMessage("只能对单条记录进行管理污染源。");
        } else {
        	window.location.href = "${root}/reg/water/pollute/list/${menuid}/"+ids+"/";
        }
    }
