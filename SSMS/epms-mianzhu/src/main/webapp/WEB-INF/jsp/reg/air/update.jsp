...
<div class="row">
    <form id="inputForm" class="form-horizontal" action="${root}/reg/air/doUpdate/${menuid }/" method="post"
                ...
                <div class="profile-info-value">
					<select id="pointId" name="pointId" style="min-width:120px; width: 350px;" class="required">
						...
						<c:forEach items="${airpoints }" var="item">
							<option value="${item.id }" ${info.pointId eq item.id ? 'selected' : '' }>${item.name }【${item.code }】</option>
						</c:forEach>
					</select>
					<span style="color: red">*</span>
				</div>
