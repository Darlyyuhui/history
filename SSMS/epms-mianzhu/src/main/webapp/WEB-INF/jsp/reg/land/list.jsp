...
<div style="margin:10px 0">
    <form class="form-search" action="${root}/reg/land/list/${menuid}/" method="post">
        <div class="row">
            <div class="col-xs-12">
                <div class="input-group">
                   	<input type="text" class="form-control search-query" id="name" name="search_name" value="${name }" placeholder="样品名称" />
...
<div id="modal-table" class="modal fade" tabindex="-1" style="top:80px;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="advanForm" role="form" class="form-search" action="${root}/reg/land/list/${menuid}/" method="post">
...
<script type="text/javascript">
    function goList() {
        window.location.href = "${root}/reg/land/list/${menuid}/?isgetsession=1";
    }
    ...
    function add() {
        window.location.href = "${root}/reg/land/add/${menuid}/?page=${page}";
    }
    function updateById(id) {
        window.location.href = "${root}/reg/land/update/${menuid}/"+id+"/?page=${page}";
    }
    function showViewById(id) {
        window.location.href = "${root}/reg/land/showView/${menuid}/"+id+"/?page=${page}";
    }
    ...
    function deleteById(ids) {
        bootbox.confirm("删除后数据将无法恢复，确定要删除吗？",function(a) {
            if (!a) return;
            var url = "${root}/reg/land/doDelete/";
            $.ajax({
                type: 'post',
                url: url,
                data : "ids="+ids,
    ...
    function checkByIds() {
        ...
        if (ids.length == 0) {
        	...
        	bootbox.confirm("确定要核查选中的数据吗？",function(a) {
                if (!a) return;
                var url = "${root}/reg/land/checkRegs/";
                $.ajax({
                    type: 'post',
                    url: url,
