...
<div class="conten_box">
    <div class="row-fluid search-area" style="padding:6px 0 4px;">
        <form class="form-inline" action="${root}/system/user/dialog/show/${menuid}/" method="post" style="margin:0;">
            <table>
                <tr>
                    ...
                    <table class="table table-striped table-bordered table-hover table-style" id="table" style="width:470px">
                        ...
                        <c:forEach items="${pageList.result}" var="user">
                            <tr onclick="rowOnclick(this)">
                                <td style="text-align:center;"><input type="radio" id="checkbox" value="${user.id}" name="select-chk" /></td>
                                <td>${user.name}</td>
                                <td>${user.account}</td>
...
<script type="text/javascript">
    ...
    function sendValue(){
        var ids = $('#table input:radio:checked').val();
        if (ids.length == 0) {
            showMessage("请选择移交人。");
            return;
        }
