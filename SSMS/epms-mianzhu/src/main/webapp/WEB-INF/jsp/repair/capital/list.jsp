...
<div style="margin:10px 0">
    <form class="form-search" action="${root}/repair/capital/list/${menuid}/" method="post">
        <div class="row">
            <div class="col-xs-12">
        ...
        <table id="table" class="table table-striped table-bordered table-hover table-style" style="text-align: center">
            ...
            <c:forEach items="${pageList.result}" var="item">
                <tr>
                    ...
                    <td>${item.proCode }</td>
                    <td>${item.proName }</td>
                    <td><tags:xiangxuncache keyName="Dic" typeCode="LAND_CAPITAL_SOURCE" id="${item.source }"/></td>
                    <td>${item.competentUnit }</td>
                    <td>${item.total }</td>
	                <td><fmt:formatDate value="${item.moneyTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
	                <td>
	                	<c:if test="${item.useStatus eq '1' }">已使用</c:if>
	                	<c:if test="${item.useStatus eq '0' }">未使用</c:if>
	                </td>
                    <td>
...
<div id="modal-table" class="modal fade" tabindex="-1" style="top:80px;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="advanForm" role="form" class="form-search" action="${root}/repair/capital/list/${menuid}/" method="post">
...
<script type="text/javascript">
    function goList() {
        window.location.href = "${root}/repair/capital/list/${menuid}/?isgetsession=1";
    }
    ...
    function add() {
        window.location.href = "${root}/repair/capital/add/${menuid}/?page=${page}";
    }
    function updateById(id) {
        window.location.href = "${root}/repair/capital/update/${menuid}/"+id+"/?page=${page}";
    }
    function showViewById(id) {
        window.location.href = "${root}/repair/capital/showView/${menuid}/"+id+"/?page=${page}";
    }
    ...
    function deleteById(ids) {
        bootbox.confirm("删除后数据将无法恢复，确定要删除吗？",function(a) {
            if (!a) return;
            var url = "${root}/repair/capital/doDelete/";
            $.ajax({
                type: 'post',
                url: url,
                data : "ids="+ids,
