...
<div class="row">
    <div class="col-sm-12" id="default-buttons">
        <h3 class="row header smaller lighter purple">
            ...
            <button class="btn btn-success " onclick="viewBtn();">
...
<script type="text/javascript">
   var zNodes = ${treeData};
    ...
    $(function () {
        ...
        var node = tree.getNodeByParam("name", "农产品种类", null);
    ...
    function addBtn() {
        if (ids == "") {
            ...
            window.location.href = "${root}/apb/apbproducttype/add/${menuid}/" + ids + "/";
        }
    }
	//修改
    function updateBtn() {
        if (ids == "") {
            ...
            window.location.href = "${root}/apb/apbproducttype/update/${menuid}/" + ids + "/";
        }
 ...
 //查看
    function viewBtn() {
        if (ids == "") {
            showMessage("请选择要查看的节点");
        } else if (ids == "0") {
        	showMessage("根节点无法查看，请选择子目录");
        } else {
            window.location.href = "${root}/apb/apbproducttype/showView/${menuid}/" + ids + "/";
        }
    	...
    	if (ids == "") {
        	...
        	if (node.isParent) {
        		...
        		bootbox.confirm("删除后数据将无法恢复，确定要删除吗？",function(a) {
                    if (!a) return;
                    var url = "${root}/apb/apbproducttype/doDelete/";
                    $.ajax({
                        type: 'post',
                        url: url,
                        data : "ids="+ids,
