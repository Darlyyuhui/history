...
<script type="text/javascript">
   var zNodes = ${treeData};
    ...
    $(function () {
        ...
        var node = tree.getNodeByParam("name", "绵竹市", null);
    ...
    function addBtn() {
        if (ids == "") {
            ...
            window.location.href = "${root}/bs/region/add/${menuid}/" + ids + "/";
        }
    }
	//修改
    function updateBtn() {
        if (ids == "") {
            ...
            window.location.href = "${root}/bs/region/update/${menuid}/" + ids + "/";
        }
    	...
    	if (ids == "") {
        	...
        	if (node.isParent) {
        		...
        		bootbox.confirm("删除后数据将无法恢复，确定要删除吗？",function(a) {
                    if (!a) return;
                    var url = "${root}/bs/region/doDelete/";
                    $.ajax({
                        type: 'post',
                        url: url,
                        data : "ids="+ids,
