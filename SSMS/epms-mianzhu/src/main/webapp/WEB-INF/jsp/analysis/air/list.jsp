...
<div style="margin:10px 0">
    <form class="form-search" action="${root}/analysis/air/list/${menuid}/" method="post">
        <div class="row">
            <div class="col-xs-12">
        ...
        <table id="table" class="table table-striped table-bordered table-hover table-style" style="text-align: center">
            ...
            <c:forEach items="${pageList.result}" var="item">
                <tr>
                    ...
                    <td><tags:xiangxuncache keyName="AIRPOINT_ID_NAME" id="${item.pointId }"/></td>
                    <td>${item.collectVolume }</td>
                    <td>${item.dept }</td>
                    <td>${item.cadmium }</td>
                    <td>${item.polluteFlux }</td>
	                <td>${item.isOver eq '1' ? '是' : '否' }</td>
	                <td>
	                	<div style="width:180px;white-space:nowrap;overflow:hidden;-o-text-overflow:ellipsis;text-overflow:ellipsis;"
                             title="${item.remark }">${item.remark }</div>
	                </td>
                    <td>
...
<div id="modal-table" class="modal fade" tabindex="-1" style="top:80px;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="advanForm" role="form" class="form-search" action="${root}/analysis/air/list/${menuid}/" method="post">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body no-padding">
                <form class="form-horizontal" id="importForm" role="form" class="form-search" 
                	action="${root}/analysis/air/doImport/${menuid}/" method="post" enctype="multipart/form-data">
                    <div class="form-group" style="margin-top:10px;">
                        <label class="col-sm-3 control-label no-padding-right"
                               for="van-search-deviceId">
                            导入文件
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <a href="${root }/downloadfile/大气沉降分析数据导入模板.xls/?filepath=download/template/excel/analysis_air_import.xls">点击下载导入模板</a>
                        </div>
                    </div>
                </form>
            </div>
...
<btn:authorBtn menuid="${menuid}" text="导出">
	<form id="exportForm" action="${root}/analysis/air/doExport/" method="post">
...
<script type="text/javascript">
    function goList() {
        window.location.href = "${root}/analysis/air/list/${menuid}/?isgetsession=1";
    }
    ...
    function add() {
        window.location.href = "${root}/analysis/air/add/${menuid}/?page=${page}";
    }
    function updateById(id) {
        window.location.href = "${root}/analysis/air/update/${menuid}/"+id+"/?page=${page}";
    }
    function showViewById(id) {
        window.location.href = "${root}/analysis/air/showView/${menuid}/"+id+"/?page=${page}";
    }
    ...
    function deleteById(ids) {
        bootbox.confirm("删除后数据将无法恢复，确定要删除吗？",function(a) {
            if (!a) return;
            var url = "${root}/analysis/air/doDelete/";
            $.ajax({
                type: 'post',
                url: url,
                data : "ids="+ids,
    ...
    function checkByIds() {
        ...
        if (ids.length == 0) {
        	...
        	bootbox.confirm("确定要核查选中的数据吗？",function(a) {
                if (!a) return;
                var url = "${root}/analysis/air/checkRegs/";
                $.ajax({
                    type: 'post',
                    url: url,
